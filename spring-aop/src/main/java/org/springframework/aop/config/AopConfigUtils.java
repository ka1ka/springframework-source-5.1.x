	...
	private static BeanDefinition registerOrEscalateApcAsRequired(
		...
		if (registry.containsBeanDefinition(AUTO_PROXY_CREATOR_BEAN_NAME)) {
			...
			if (!cls.getName().equals(apcDefinition.getBeanClassName())) {
				// 根据class获取基础设施Bean对应的优先级。此处是预先将处理aop的基础设施Class放入到了一个List中，然后获取优先级的时候获取的是下标
		...
		// 创建一个AnnotationAwareAspectJAutoProxyCreator类型的Bean定义，并注册到容器中.
		RootBeanDefinition beanDefinition = new RootBeanDefinition(cls);
		beanDefinition.setSource(source);
		// 设置最高优先级。order值越小，优先级越大
		beanDefinition.getPropertyValues().add("order", Ordered.HIGHEST_PRECEDENCE);
		// 设置bean定义的角色，表示是一个基础设施Bean。
		beanDefinition.setRole(BeanDefinition.ROLE_INFRASTRUCTURE);

		// 设置bean定义的名称为：‘org.springframework.aop.config.internalAutoProxyCreator’
		//  注册AnnotationAwareAspectJAutoProxyCreator类型的bean定义.
